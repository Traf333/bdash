...
export async function copyToClipboard(accounts: string[]) {
  console.log("selected accounts: ", accounts.length);
  const textToCopy = content + `processAccounts(${JSON.stringify(accounts)})`;
  try {
