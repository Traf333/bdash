<script lang="ts">
  ...
  async function refreshAll() {
    isRotating = true;
    await Promise.all(filteredUsers.map(refresh));
    isRotating = false;
  }
  ...
  $: filteredUsers = $users.filter((u) => {
    const matchesSearch = search
      ...
      : true;
    const matchesStatus =
      typeof selectedStatus === "boolean" ? u.status === selectedStatus : true;
    ...
    <Toolbar embedded class="w-full py-4 text-gray-500  dark:text-gray-400">
      ...
      <div class="border-l border-gray-100 pl-2 dark:border-gray-700">
        <ToolbarButton
          ...
          on:click={refreshAll}
        >
          <RefreshOutline size="lg" class={isRotating ? "isRotating" : ""} />
        </ToolbarButton>
        <ToolbarButton
          color="dark"
          class="m-0 rounded p-1 hover:bg-gray-100 focus:ring-0 dark:hover:bg-gray-700"
          on:click={() =>
            copyToClipboard(
              filteredUsers.filter((u) => u.selected).map((u) => u.access_token)
            )}
        >
      ...
      <div slot="end" class="flex items-center space-x-2">
        <Button
          ...
          on:click={() => ((current_user = undefined), (openUser = true))}
        >
      ...
      {#each sortUsers(filteredUsers) as user}
        <TableBodyRow class="text-base">
          <TableBodyCell class="w-4 p-4"
            ><Checkbox bind:checked={user.selected} /></TableBodyCell
          >
          ...
          <TableBodyCell class="w-2 space-x-2 p-4">
            ...
            <Button size="sm" class="gap-2 px-3" on:click={() => refresh(user)}>
